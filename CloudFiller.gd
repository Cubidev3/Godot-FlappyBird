...
func fill_screen():
	...
	var end_point = Vector2(cam.global_position.x + screen_size.x / 2, cam.global_position.y - screen_size.y / 2)
	
	var x_scale = abs(end_point.x - start_point.x) / sprite_side
	var y_scale = abs(end_point.y - start_point.y) / sprite_side
	
	self.global_position = Vector2(end_point.x + start_point.x, end_point.y + start_point.y) / 2
	scale.x = x_scale
	scale.y = y_scale
